...
export default function HospitalsOrderPage() {
    const { t } = useLanguage();

    const translations = {
        title: { en: 'Medical Uniforms Order', ar: 'طلب الأزياء الطبية' },
        subtitle: {
            en: 'Fill out the form below to request a quote for medical scrubs and healthcare uniforms',
            ar: 'املأ النموذج أدناه لطلب عرض أسعار للأزياء الطبية وزي العاملين الصحيين'
        },
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2">
                        <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12">
                            ...
                            <DynamicForm
                                questions={questionsConfig.hospitals}
                                sector="hospitals"
                            />
                        </div>
                    </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-xl p-6 sticky top-24">
                            ...
                            <div className="mt-8 pt-6 border-t border-gray-200">
                                <h3 className="text-sm font-semibold text-gray-900 mb-3">
                                    {t({ en: 'Medical-Grade Features', ar: 'ميزات طبية احترافية' })}
                                </h3>
                                <ul className="space-y-2 text-sm text-gray-600">
                                    <li className="flex items-start space-x-2 rtl:space-x-reverse">
                                        <span className="text-green-500 mt-0.5">✓</span>
                                        <span>{t({ en: 'Antimicrobial fabrics', ar: 'أقمشة مضادة للميكروبات' })}</span>
                                    </li>
                                    <li className="flex items-start space-x-2 rtl:space-x-reverse">
                                        <span className="text-green-500 mt-0.5">✓</span>
                                        <span>{t({ en: 'Fluid resistant materials', ar: 'مواد مقاومة للسوائل' })}</span>
                                    </li>
                                    <li className="flex items-start space-x-2 rtl:space-x-reverse">
                                        <span className="text-green-500 mt-0.5">✓</span>
                                        <span>{t({ en: 'Breathable & comfortable', ar: 'مريحة وجيدة التهوية' })}</span>
                                    </li>
                                    <li className="flex items-start space-x-2 rtl:space-x-reverse">
                                        <span className="text-green-500 mt-0.5">✓</span>
                                        <span>{t({ en: 'Easy care & wrinkle-free', ar: 'سهلة العناية ومقاومة للتجاعيد' })}</span>
                                    </li>
                                </ul>
                            </div>
                        </div>
