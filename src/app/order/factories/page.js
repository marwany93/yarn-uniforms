...
export default function FactoriesOrderPage() {
    const { t } = useLanguage();

    const translations = {
        title: { en: 'Factory Workwear Order', ar: 'طلب ملابس المصانع' },
        subtitle: {
            en: 'Fill out the form below to request a quote for factory workwear and safety uniforms',
            ar: 'املأ النموذج أدناه لطلب عرض أسعار لملابس العمل وأزياء السلامة للمصانع'
        },
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2">
                        <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12">
                            ...
                            <DynamicForm
                                questions={questionsConfig.factories}
                                sector="factories"
                            />
                        </div>
                    </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-xl p-6 sticky top-24">
                            ...
                            <div className="mt-8 pt-6 border-t border-gray-200">
                                <h3 className="text-sm font-semibold text-gray-900 mb-3">
                                    {t({ en: 'Safety Features Available', ar: 'ميزات السلامة المتاحة' })}
                                </h3>
                                <ul className="space-y-2 text-sm text-gray-600">
                                    <li className="flex items-start space-x-2 rtl:space-x-reverse">
                                        <span className="text-orange-500 mt-0.5">✓</span>
                                        <span>{t({ en: 'Fire resistant materials', ar: 'مواد مقاومة للحريق' })}</span>
                                    </li>
                                    <li className="flex items-start space-x-2 rtl:space-x-reverse">
                                        <span className="text-orange-500 mt-0.5">✓</span>
                                        <span>{t({ en: 'High visibility options', ar: 'خيارات عالية الوضوح' })}</span>
                                    </li>
                                    <li className="flex items-start space-x-2 rtl:space-x-reverse">
                                        <span className="text-orange-500 mt-0.5">✓</span>
                                        <span>{t({ en: 'Anti-static fabrics', ar: 'أقمشة مضادة للكهرباء الساكنة' })}</span>
                                    </li>
                                    <li className="flex items-start space-x-2 rtl:space-x-reverse">
                                        <span className="text-orange-500 mt-0.5">✓</span>
                                        <span>{t({ en: 'Custom logo embroidery', ar: 'تطريز شعار مخصص' })}</span>
                                    </li>
                                </ul>
                            </div>
                        </div>
